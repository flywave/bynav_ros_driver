#include <bynav_gps_driver/gnss_spp.h>
...
void psr_res(const Eigen::Matrix<double, 7, 1> &rcv_state,
  ...
  for (uint32_t i = 0; i < num_sv; ++i) {
    ...
    if (rcv_state.topLeftCorner<3, 1>().norm() > 0) {
      ...
      ion_delay =
          calculate_ion_delay(sat_state->ttx, iono_params, rcv_lla, azel);
    }
    ...
    double sagnac_term =
        ...
        LIGHT_SPEED;
    double psr_estimated = rv2sv.norm() + sagnac_term +
                           rcv_state(3 + sys2idx.at(this_sys)) -
...
Eigen::Matrix<double, 7, 1> psr_pos(const std::vector<ObsPtr> &obs,
  ...
  while (num_iter < MAX_ITER_PVT && dx_norm > EPSILON_PVT) {
    ...
    std::vector<Eigen::Vector2d> all_sv_azel;
    psr_res(xyzt, valid_obs, all_sat_states, iono_params, b, G, atmos_delay,
            all_sv_azel);

    std::vector<uint32_t> good_idx;
    for (uint32_t i = 0; i < valid_obs.size(); ++i) {
      if (G.row(i).norm() <= 0)
        continue; // res not computed
    ...
    for (uint32_t k = 0; k < 4; ++k)
      num_extra_constraint -= sys_mask[k];
    ...
    Eigen::MatrixXd good_W(good_num + num_extra_constraint,
                           good_num + num_extra_constraint);
    good_W.setZero();
    for (uint32_t i = 0; i < good_num; ++i) {
      ...
      int l1_idx = -1;
      L1_freq(this_obs, &l1_idx);
      // LOG_IF(FATAL, l1_idx < 0)
      //    << "[gnss_comm::psr_pos] no L1 observation found.\n";
  ...
  if (num_iter == MAX_ITER_PVT) {
    // LOG(WARNING)
    //    << "[gnss_comm::psr_pos] XYZT solver reached maximum iterations.\n";
    return result;
  }
...
void dopp_res(const Eigen::Matrix<double, 4, 1> &rcv_state,
  ...
  for (uint32_t i = 0; i < num_sv; ++i) {
    ...
    double dopp_estimated =
        (sat_state->vel - rcv_state.topLeftCorner<3, 1>()).dot(unit_rv2sv) +
...
Eigen::Matrix<double, 4, 1> dopp_vel(const std::vector<ObsPtr> &obs,
  ...
  if (ref_ecef.norm() == 0) {
    ...
    Eigen::Matrix<double, 7, 1> psr_result =
        psr_pos(obs, ephems, zero_iono_params);
    if (psr_result.head<3>().norm() != 0) {
      ref_ecef = psr_result.head<3>();
    } else {
      // LOG(ERROR) << "[gnss_comm::dopp_vel] Unable to initialize reference "
      //              "position for Doppler calculation.\n";
      return result;
    }
  }
  ...
  if (valid_obs.size() < 4) {
    // LOG(ERROR) << "[gnss_comm::dopp_vel] GNSS observation not enough for "
  ...
  while (num_iter < MAX_ITER_PVT && dx_norm > EPSILON_PVT) {
    ...
    for (uint32_t i = 0; i < good_num; ++i) {
      ...
      L1_freq(this_obs, &l1_idx);
      // LOG_IF(FATAL, l1_idx < 0)
      //    << "[gnss_comm::dopp_vel] no L1 observation found.\n";
  ...
  if (num_iter == MAX_ITER_PVT) {
    // LOG(WARNING)
    //     << "[gnss_comm::dopp_vel] XYZT solver reached maximum iterations.\n";
  }
