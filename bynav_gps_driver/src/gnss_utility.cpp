...
Eigen::Vector3d geo2ecef(const Eigen::Vector3d &lla) {
  ...
  const double N =
      EARTH_SEMI_MAJOR / std::sqrt(1 - EARTH_ECCE_2 * sin_lat * sin_lat);
...
Eigen::Vector3d eph2pos(const gtime_t &curr_time, const EphemPtr ephem_ptr,
  ...
  case SYS_GLO:
    mu = MU;
    earth_omg = EARTH_OMG_GLO;
    break;
  case SYS_BDS:
    mu = MU;
    earth_omg = EARTH_OMG_BDS;
    break;
  }

  double n0 =
      sqrt(mu / pow(ephem_ptr->A, 3)); // mean motion angular velocity (rad/sec)
  double n = n0 + ephem_ptr->delta_n;  // corrected mean motion
  double Mk = ephem_ptr->M0 + n * tk;
  double Ek =
      Kepler(Mk, ephem_ptr->e); // solve Kepler equation for eccentric anomaly
  double sin_Ek = sin(Ek), cos_Ek = cos(Ek);
  double vk = atan2(sqrt(1 - ephem_ptr->e * ephem_ptr->e) * sin_Ek,
                    cos_Ek - ephem_ptr->e);
  double phi = vk + ephem_ptr->omg;
  double cos_2phi = cos(2 * phi);
  double sin_2phi = sin(2 * phi);

  double delta_uk = ephem_ptr->cus * sin_2phi +
                    ephem_ptr->cuc * cos_2phi; // latitude correction
  double delta_rk = ephem_ptr->crs * sin_2phi +
                    ephem_ptr->crc * cos_2phi; // radius correction
  double delta_ik = ephem_ptr->cis * sin_2phi +
                    ephem_ptr->cic * cos_2phi; // correction to inclination
  double uk = phi + delta_uk;                  // corrected latitude
  double rk =
      ephem_ptr->A * (1 - ephem_ptr->e * cos_Ek) + delta_rk; // corrected radius
  double ik =
      ephem_ptr->i0 + ephem_ptr->i_dot * tk + delta_ik; // corrected inclination
  ...
  double toe_tow =
      (sys == SYS_BDS ? time2bdt(time_add(ephem_ptr->toe, -14), NULL)
                      : ephem_ptr->toe_tow);

  if (sys == SYS_BDS && prn <= 5) // BDS GEO satellite
  {
    double OMG_k =
        ephem_ptr->OMG0 + ephem_ptr->OMG_dot * tk - earth_omg * toe_tow;
  ...
  dts -= 2.0 * sqrt(mu * ephem_ptr->A) * ephem_ptr->e * sin_Ek / LIGHT_SPEED /
         LIGHT_SPEED;
...
Eigen::Vector3d eph2vel(const gtime_t &curr_time, const EphemPtr ephem_ptr,
                        double *svddt) {
  ...
  double vk_dot = sqrt(1 - ephem_ptr->e * ephem_ptr->e) * Ek_dot /
                  (1 - ephem_ptr->e * cos_Ek);
  double vk = atan2(sqrt(1 - ephem_ptr->e * ephem_ptr->e) * sin_Ek,
                    cos_Ek - ephem_ptr->e);
  double phi = vk + ephem_ptr->omg;
  double cos_2phi = cos(2 * phi);
  ...
  double delta_uk_dot =
      2 * vk_dot * (ephem_ptr->cus * cos_2phi - ephem_ptr->cuc * sin_2phi);
  double delta_rk_dot =
      2 * vk_dot * (ephem_ptr->crs * cos_2phi - ephem_ptr->crc * sin_2phi);
  double delta_ik_dot =
      2 * vk_dot * (ephem_ptr->cis * cos_2phi - ephem_ptr->cic * sin_2phi);
  ...
  if (sys == SYS_BDS && prn <= 5) // BDS GEO satellite
  {
    ...
    double term2 = xk_prime * OMGk_dot + yk_prime_dot * cos_ik -
                   yk_prime * ik_dot * sin_ik;
    double xg = xk_prime * cos_OMG_k - yk_prime * cos_ik * sin_OMG_k;
    double yg = xk_prime * sin_OMG_k + yk_prime * cos_ik * cos_OMG_k;
    double zg = yk_prime * sin_ik;
    ...
    sv_vel.x() = xg_dot * cos_o + xg * cos_o_dot + yg_dot * sin_o * COS_N5 +
                 yg * sin_o_dot * COS_N5 + zg_dot * sin_o * SIN_N5 +
                 zg * sin_o_dot * SIN_N5;
    sv_vel.y() = -xg_dot * sin_o - xg * sin_o_dot + yg_dot * cos_o * COS_N5 +
                 yg * cos_o_dot * COS_N5 + zg_dot * cos_o * SIN_N5 +
                 zg * cos_o_dot * SIN_N5;
  ...
  // relativity correction
  ddts -= 2.0 * sqrt(mu * ephem_ptr->A) * ephem_ptr->e * cos_Ek * Ek_dot /
...
void deq(const Eigen::Vector3d &pos, const Eigen::Vector3d &vel,
  ...
  double a = 1.5 * J2_GLO * MU * EARTH_SEMI_MAJOR_GLO * EARTH_SEMI_MAJOR_GLO /
             r2 / r3;                      /* 3/2*J2*mu*Ae^2/r^5 */
...
Eigen::Vector3d ecef2enu(const Eigen::Vector3d &ref_lla,
  ...
  R_enu_ecef << -sin_lon, cos_lon, 0, -sin_lat * cos_lon, -sin_lat * sin_lon,
...
Eigen::Matrix3d geo2rotation(const Eigen::Vector3d &ref_geo) {
  ...
  R_ecef_enu << -sin_lon, -sin_lat * cos_lon, cos_lat * cos_lon, cos_lon,
      -sin_lat * sin_lon, cos_lat * sin_lon, 0, cos_lat, sin_lat;
  return R_ecef_enu;
}
...
void sat_azel(const Eigen::Vector3d &rev_pos, const Eigen::Vector3d &sat_pos,
  ...
  azel[0] = rev2sat_ecef.head<2>().norm() < 1e-12
                ? 0.0
                : atan2(rev2sat_enu.x(), rev2sat_enu.y());
 ...
 *------------------------------ calculate troposphere hydrostatic/wet mapping
 *functions by Niell model ref: Global mapping functions for the atmosphere
 *delay at radio wavelengths args   : gtime_t time                 I   time
 *(gpst) args   : Eigen::Vector3d rev_lla      I   receiver position in geodetic
 *coordinate double*         azel         I   satellite azimuth/elevation angle
 *in radius double*         mapfh        IO  hydrostatic mapping coefficient
 * return : delay expressed by light travel distance (m)
 *-----------------------------------------------------------------------------*/
static void nmf(gtime_t time, const Eigen::Vector3d rev_lla,
                const double azel[], double *mapfh, double *mapfw) {
...
double calculate_trop_delay(gtime_t time, const Eigen::Vector3d &rev_lla,
  ...
  if (rev_lla.z() < -100.0 || 1E4 < rev_lla.z() || azel[1] <= 0)
    return 0.0;
  ...
  double zhd =
      0.0022768 * pres /
      (1.0 - 0.00266 * cos(2.0 * rev_lla.x() * D2R) - 0.00028 * hgt / 1E3);
...
double calculate_ion_delay(gtime_t t, const std::vector<double> &ion_parameters,
  ...
  if (ion_parameters.empty())
    return 0.0;
  if (rev_lla.z() < -1E3 || azel[1] <= 0)
    return 0.0;
  ...
  /* ionospheric delay */
  double amp = ion_parameters[0] +
               phi * (ion_parameters[1] +
                      phi * (ion_parameters[2] + phi * ion_parameters[3]));
  double per = ion_parameters[4] +
               phi * (ion_parameters[5] +
                      phi * (ion_parameters[6] + phi * ion_parameters[7]));
  amp = amp < 0.0 ? 0.0 : amp;
  per = per < 72000.0 ? 72000.0 : per;
  double x = 2.0 * M_PI * (tt - 50400.0) / per;

  return LIGHT_SPEED * f *
