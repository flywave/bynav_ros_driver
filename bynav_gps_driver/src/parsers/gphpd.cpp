...
bynav_gps_msgs::GphpdPtr bynav_gps_driver::GphpdParser::ParseAscii(
  ...
  valid = valid && ParseFloat(sentence.body[2], msg->seconds);
  ...
  valid = valid && ParseDouble(sentence.body[9], msg->east_distance);
  valid = valid && ParseDouble(sentence.body[10], msg->north_distance);
  valid = valid && ParseDouble(sentence.body[11], msg->up_distance);
  ...
  valid = valid && ParseDouble(sentence.body[15], msg->east_velocity_diff);
  valid = valid && ParseDouble(sentence.body[16], msg->north_velocity_diff);

  valid = valid && ParseDouble(sentence.body[17], msg->baseline_length);

  valid = valid && ParseUInt8(sentence.body[18], msg->num_satellites_ant1);
  valid = valid && ParseUInt8(sentence.body[19], msg->num_satellites_ant2);

  uint8_t solution_status;
  valid = valid && ParseUInt8(sentence.body[20], solution_status);
