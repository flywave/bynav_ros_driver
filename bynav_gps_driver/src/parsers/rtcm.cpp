...
bool Rtcm::ReadCB(uint8_t byte) {
  ...
  case GOT_LENGTH2:
    in_buffer_.buf[buffer_head_++] = byte;
    if (buffer_head_ - 3 == payload_len_) {
...
void Rtcm::Decode() {
  ...
  if (CheckCRC()) {
    ...
    msg_type_ = (uint16_t)(in_buffer_.buf[3] << 4) |
                uint16_t(in_buffer_.buf[4] & 0xF0) >> 4;

    for (auto &cb : buffer_callbacks_)
      cb(in_buffer_.buf, message_len_, msg_type_,
         ((uint32_t)(ck_a_ << 16) | (uint32_t)(ck_b_ << 8) | (uint32_t)ck_c_));
  } else {
    num_errors_++;
  }
}
...
bool Rtcm::CheckCRC() {
  ...
  for (int i = 0; i < buffer_head_ - 3; i++) {
    crc =
        ((crc << 8) & 0xFFFFFF) ^ CRC24_TABLE[(crc >> 16) ^ in_buffer_.buf[i]];
  }
