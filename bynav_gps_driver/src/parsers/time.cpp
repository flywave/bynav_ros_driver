...
const std::string bynav_gps_driver::TimeParser::MESSAGE_NAME = "TIME";

uint32_t bynav_gps_driver::TimeParser::GetMessageId() const {
  return MESSAGE_ID;
}

const std::string bynav_gps_driver::TimeParser::GetMessageName() const {
  return MESSAGE_NAME;
}

bynav_gps_msgs::TimePtr bynav_gps_driver::TimeParser::ParseBinary(
    const bynav_gps_driver::BinaryMessage &msg) noexcept(false) {
  ...
  bynav_gps_msgs::TimePtr ros_msg = boost::make_shared<bynav_gps_msgs::Time>();
...
bynav_gps_msgs::TimePtr bynav_gps_driver::TimeParser::ParseAscii(
    const bynav_gps_driver::BynavSentence &sentence) noexcept(false) {
  bynav_gps_msgs::TimePtr msg = boost::make_shared<bynav_gps_msgs::Time>();
  if (sentence.body.size() != ASCII_FIELD) {
    std::stringstream error;
    error << "Unexpected number of fields in TIME log: "
          << sentence.body.size();
    throw ParseException(error.str());
  }
  bool valid = true;
