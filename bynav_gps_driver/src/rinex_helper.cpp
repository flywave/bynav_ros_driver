...
static GloEphemPtr
rinex_line2glo_ephem(const std::vector<std::string> &ephem_lines,
                     const uint32_t gpst_leap_seconds) {
  // LOG_IF(FATAL, ephem_lines.size() != 4)
  //    << "GLO ephemeris record should contain 8 lines";
  // LOG_IF(FATAL, ephem_lines[0].at(0) != 'R')
  //    << "Not a valid GLO ephemeris record";
  ...
  glo_ephem->health =
      static_cast<uint32_t>(str2double(ephem_lines[1].substr(61, 19)));
  ...
  glo_ephem->freqo =
      static_cast<int>(str2double(ephem_lines[2].substr(61, 19)));
  ...
  glo_ephem->age =
      static_cast<uint32_t>(str2double(ephem_lines[3].substr(61, 19)));

  return glo_ephem;
}

static EphemPtr rinex_line2ephem(const std::vector<std::string> &ephem_lines) {
  // LOG_IF(FATAL, ephem_lines.size() != 8)
  //    << "Ephemeris record should contain 8 lines";
...
void rinex2ephems(const std::string &rinex_filepath,
  ...
  while (std::getline(ephem_file, line)) {
    if (line.at(0) == 'G' || line.at(0) == 'C' || line.at(0) == 'E') {
      ...
      GloEphemPtr glo_ephem =
          rinex_line2glo_ephem(ephem_lines, gpst_leap_seconds);
...
rinex_line2obs(const std::string rinex_str,
  ...
  for (auto type : date_types) {
    ...
    line_offset += 14 + 2;
    if (field_str.find_first_not_of(' ') == std::string::npos)
      continue;
  ...
  // LOG_IF(FATAL, num_freqs < obs->status.size())
  //   << "Suspicious observation field.\n";
...
void rinex2obs(const std::string &rinex_filepath,
  ...
  while (std::getline(obs_file, rinex_line)) {
    // LOG_IF(FATAL, rinex_line.at(0) != '>')
    //     << "Invalid Observation record " << rinex_line;
    // LOG_IF(FATAL, rinex_line.at(31) != '0')
    //     << "Invalid Epoch data " << rinex_line.at(31) - 48;
    ...
    for (int i = 0; i < num_obs; ++i) {
      // LOG_IF(FATAL, !std::getline(obs_file, rinex_line))
      //    << "Incomplete RINEX file";
      ObsPtr obs = rinex_line2obs(rinex_line, sys2type);
      if (!obs || obs->freqs.empty())
        continue;
      obs->time = obs_time;
...
void rinex2iono_params(const std::string &rinex_filepath,
  ...
  if (!(std::getline(file, line) &&
        line.find("RINEX VERSION") != std::string::npos &&
        line.find("3.04") != std::string::npos)) {
...
get_glo_freqo(const std::vector<std::vector<ObsPtr>> &gnss_meas) {
  ...
  for (auto &meas : gnss_meas) {
    for (auto &obs : meas) {
      ...
      for (double freq : obs->freqs) {
        if (freq > FREQ1_GLO - 8 * DFRQ1_GLO &&
            freq < FREQ1_GLO + 7 * DFRQ1_GLO) {
          const int freqo = round((freq - FREQ1_GLO) / DFRQ1_GLO);
          sat2freqo.emplace(obs->sat, freqo);
        } else if (freq > FREQ2_GLO - 8 * DFRQ2_GLO &&
                   freq < FREQ2_GLO + 7 * DFRQ2_GLO) {
...
void obs2rinex(const std::string &rinex_filepath,
  ...
  fprintf(fp, "%c  %3d %3s %3s %3s %3s %3s %3s %3s %3s%20s  %-20s\n", 'G', 8,
          "C1C", "L1C", "D1C", "S1C", "C2S", "L2S", "D2S", "S2S", "",
          "SYS / # / OBS TYPES ");
  fprintf(fp, "%c  %3d %3s %3s %3s %3s %3s %3s %3s %3s%20s  %-20s\n", 'R', 8,
          "C1C", "L1C", "D1C", "S1C", "C2C", "L2C", "D2C", "S2C", "",
          "SYS / # / OBS TYPES ");
  fprintf(fp, "%c  %3d %3s %3s %3s %3s %3s %3s %3s %3s%20s  %-20s\n", 'E', 8,
          "C1C", "L1C", "D1C", "S1C", "C7Q", "L7Q", "D7Q", "S7Q", "",
          "SYS / # / OBS TYPES ");
  fprintf(fp, "%c  %3d %3s %3s %3s %3s %3s %3s %3s %3s%20s  %-20s\n", 'C', 8,
          "C2I", "L2I", "D2I", "S2I", "C7I", "L7I", "D7I", "S7I", "",
          "SYS / # / OBS TYPES ");
  ...
  for (int i = 0; i < (num_glo_sats <= 0 ? 1 : (num_glo_sats - 1) / 8 + 1);
       ++i) {
    if (i == 0)
    ...
    for (int j = 0; j < 8; ++j) {
      if (i * 8 + j < num_glo_sats) {
        fprintf(fp, " %3s %2d", sat2str(glo_sat2freqo_it->first).c_str(),
  ...
  fprintf(fp, " C1C    0.000 C1P    0.000 C2C    0.000 C2P    0.000        "
              "GLONASS COD/PHS/BIS \n");
  fprintf(fp, "%-60.60s%-20s\n", "", "END OF HEADER");
  ...
  for (auto &meas : gnss_meas) {
    ...
    fprintf(fp, "> %04.0f %2.0f %2.0f %2.0f %2.0f%11.7f  %d%3d%21s\n", ep[0],
            ep[1], ep[2], ep[3], ep[4], ep[5], 0, static_cast<int>(meas.size()),
            "");

    std::vector<ObsPtr> sorted_meas(meas);
    std::sort(sorted_meas.begin(), sorted_meas.end(),
              [](ObsPtr o1, ObsPtr o2) { return o1->sat < o2->sat; });
